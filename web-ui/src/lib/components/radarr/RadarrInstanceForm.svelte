<script lang="ts">
  ...
  import { createRadarrInstance, updateRadarrInstance, testRadarrInstance, getRadarrRootFolders, getRadarrQualityProfiles } from '$lib/api/radarr';
  import { showToast } from '$lib/state/toast-state';
  import type { RadarrInstanceResponse, RadarrRootFolder, RadarrQualityProfile } from '$lib/api/types';
  ...
  let rootFolders: RadarrRootFolder[] = [];
  let qualityProfiles: RadarrQualityProfile[] = [];
  ...
  onMount(() => {
    if (instance) {
      ...
      radarrImportPath = instance.radarrImportPath;
  ...
  async function handleTest() {
    ...
    try {
      if (instance) {
        const resp = await testRadarrInstance(instance.id, {
        ...
        const resp = await testRadarrInstance('_new_', {
          baseUrl: baseUrl || undefined,
          apiKey: apiKey || undefined,
          tubeWritePath: tubeWritePath || undefined,
        });
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (isEdit && instance) {
        await updateRadarrInstance(instance.id, body);
        showToast(`Updated ${name}`);
      } else {
        const created = await createRadarrInstance(body as any);
...
<div class="page">
  <header class="page-head">
    <div>
      <div class="crumbs">
        <button class="crumb-btn" type="button" on:click={goBack}>← Back to Radarr Instances</button>
      </div>
      <h1 class="page-title">{isEdit ? 'Edit Radarr Instance' : 'Add Radarr Instance'}</h1>
      <p class="page-subtitle">Connect a Radarr server and configure Docker-safe path mapping.</p>
    </div>
    <div class="toolbar">
    ...
    <form class="card" on:submit|preventDefault={handleSave}>
      ...
      <div class="form-grid">
        <div class="form-group">
          ...
          <input class="input" bind:value={name} placeholder="Main Radarr" required />
          <div class="hint">Friendly name shown in selectors and missing movie pages.</div>
        </div>
        <div class="form-group">
          <label>Base URL <span class="required">*</span></label>
          <input class="input" bind:value={baseUrl} placeholder="http://radarr-main:7878" required />
          <div class="hint">Include protocol and port. Example: http://radarr:7878</div>
        </div>
      ...
      <div class="form-grid">
        <div class="form-group">
          ...
          <input class="input" bind:value={tubeWritePath} placeholder="/downloads/radarr-main" required />
          <div class="hint">Path inside the Tube Explore container where files are downloaded.</div>
        </div>
        <div class="form-group">
          <label>Radarr Import Path <span class="required">*</span></label>
          <input class="input" bind:value={radarrImportPath} placeholder="/data/imports/tube-explore/radarr-main" required />
          <div class="hint">Path inside the Radarr container pointing to the same physical directory.</div>
        </div>
        <div class="form-group full">
          <label>Host Path Hint</label>
          <input class="input" bind:value={hostPathHint} placeholder="/mnt/media/radarr" />
      ...
      <div class="notice-box">
        ...
        Both paths must point to the same physical host directory. They may be different because Tube Explore and Radarr run in different containers.
      </div>
      ...
      <div class="form-grid">
        <div class="form-group">
          ...
          <select class="input" bind:value={defaultQualityProfileId}>
            <option value={undefined}>— Use Radarr default —</option>
      ...
      <div class="side-card">
        ...
        <div class="steps">
          ...
          <div class="step">
            ...
            <div>
              <b>Import</b>
              <div class="sub">Radarr scans the mapped import path.</div>
            </div>
          </div>
          <div class="step">
            ...
            <div>
              <b>Organize</b>
              <div class="sub">Radarr moves/renames the file into the library.</div>
            </div>
          </div>
        </div>
      </div>
