<script lang="ts">
  ...
  import { downloadForArr } from '$lib/api/arr';
  import type { ArrInstanceResponse, ArrMissingItem } from '$lib/api/types';

  export let navigate: (page: string, data?: any) => void = () => {};
  export let instance: ArrInstanceResponse;
  export let episode: ArrMissingItem;
  export let presetUrl = '';

  let query = episode?.title || '';
  let results: Array<{
    title: string;
    source: string;
    channel: string;
  ...
  async function submitPasteUrl() {
    ...
    try {
      ...
      showToast(`Download task created for Sonarr`);
      closePasteModal();
    } catch (e) {
  ...
  async function downloadResult(result: typeof results[0]) {
    try {
      const resp = await downloadForArr({
        url: result.url,
        instanceId: instance?.id,
        itemId: episode?.itemId,
        itemTitle: episode?.title,
...
<div class="page">
  <header class="page-head">
    <div>
      <div class="crumbs">
        ...
        <span class="crumb">Missing Episodes</span> ›
        <span>Search</span>
      </div>
      <h1 class="page-title">Search for <span class="highlight">{episode?.title}</span></h1>
      <p class="page-subtitle">Find and select a video to download and import to Sonarr.</p>
    </div>
    <div class="toolbar">
  ...
  <section class="top-strip">
    <div class="episode-cell">
      ...
      <div>
        <div class="summary-label">Target Episode</div>
        <div class="item-title">{episode?.title}</div>
        <div class="sub">{episode?.seriesTitle} S{String(episode?.seasonNumber ?? '?').padStart(2, '0')}E{String(episode?.episodeNumber ?? '?').padStart(2, '0')}</div>
      </div>
    </div>
    <div>
    ...
    <div>
      <div class="summary-label">Import Path (Sonarr view)</div>
      <div class="path">{instance?.arrImportPath}</div>
  ...
  <section class="panel">
    ...
    {#if !searched}
      <div class="empty-state">
        ...
        <p>Enter a search query or paste a URL to find a video for this episode.</p>
      </div>
  ...
  <div class="modal-backdrop" on:click|self={closePasteModal}>
    <div class="modal">
      <div class="modal-head">
        <div class="modal-title">Paste URL for {episode?.title}</div>
...
<style>
  ...
  .secondary-btn { height: 48px; border-radius: 14px; padding: 0 18px; display: inline-flex; align-items: center; gap: 10px; color: white; background: rgba(255,255,255,.045); border: 1px solid rgba(255,255,255,.1); cursor: pointer; font-weight: 800; white-space: nowrap; }
  .primary-btn { height: 48px; border: 0; border-radius: 15px; padding: 0 22px; display: inline-flex; align-items: center; gap: 10px; color: white; cursor: pointer; font-weight: 850; font-size: 15px; background: linear-gradient(180deg, #956aff 0%, #6c35ff 56%, #4f22d8 100%); white-space: nowrap; }
  .primary-btn:disabled { opacity: 0.5; cursor: default; }
  .top-strip { display: grid; grid-template-columns: 1.1fr 1fr 1.3fr 1.3fr; gap: 18px; padding: 18px; border-radius: 18px; background: linear-gradient(180deg, rgba(255,255,255,.06), rgba(255,255,255,.025)), rgba(8,13,34,.64); border: 1px solid rgba(255,255,255,.105); margin-bottom: 18px; }
  .episode-cell { display: flex; gap: 14px; align-items: center; }
  .poster { width: 60px; height: 82px; border-radius: 8px; border: 1px solid rgba(255,255,255,.08); flex: 0 0 auto; }
  ...
  .path { font-family: monospace; font-size: 13px; background: rgba(255,255,255,.05); padding: 2px 6px; border-radius: 4px; }
  ...
  .section-head { display: flex; align-items: center; justify-content: space-between; padding: 16px 18px 8px; }
  .section-head h2 { margin: 0; font-size: 16px; letter-spacing: -0.03em; }
  .result-grid { padding: 6px 10px 14px; display: grid; gap: 2px; }
  .result-row { display: grid; grid-template-columns: 100px minmax(0, 1fr) auto; gap: 14px; align-items: center; padding: 10px 12px; border-radius: 14px; transition: 180ms ease; }
  .result-row:hover { background: rgba(255,255,255,.026); }
  .result-thumb { height: 60px; border-radius: 10px; border: 1px solid rgba(255,255,255,.08); }
  .actions { display: flex; gap: 8px; align-items: center; }
  .badge { padding: 3px 9px; border-radius: 8px; font-size: 12px; font-weight: 850; }
  ...
  .icon-btn { width: 34px; height: 34px; border-radius: 9px; border: 1px solid rgba(255,255,255,.09); background: rgba(255,255,255,.045); color: var(--muted); cursor: pointer; display: grid; place-items: center; }
  .modal-actions { margin-top: 18px; display: flex; justify-content: flex-end; gap: 12px; }
  @media (max-width: 1240px) { .top-strip { grid-template-columns: 1fr 1fr; } .result-row { grid-template-columns: 80px 1fr; } .result-row .actions { grid-column: 2; } }
  @media (max-width: 900px) { .page-head { grid-template-columns: 1fr; } .top-strip { grid-template-columns: 1fr; } }
</style>
